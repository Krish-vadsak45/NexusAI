...
import { GripVertical, Plus, FileDown, FileText, Trash2 } from "lucide-react";
...
export function ProjectAssembler({ items }: ProjectAssemblerProps) {
  ...
  const { setNodeRef: setCanvasAreaRef } = useDroppable({ id: "canvas-area" });
          ...
          if (clonedElement && originalElement) {
            ...
            const bakeStyles = (source: Element, target: HTMLElement) => {
              ...
              props.forEach((prop) => {
                ...
                >;
                const computedValue =
                  computed[prop as keyof CSSStyleDeclaration];
                if (typeof computedValue === "string" && computedValue) {
                  styleTarget[prop] = computedValue;
                }
              });
  ...
  const exportToMarkdown = () => {
    ...
    canvasItems.forEach((item) => {
      ...
      if (tool === "Article Writer") {
        markdown +=
          String((output as { article?: string }).article || "") + "\n\n";
      } else if (tool === "Code Generator") {
        markdown +=
          "```" +
          String((item.historyItem.input as UnknownRecord).language || "") +
          "\n";
        markdown += String((output as { code?: string }).code || "") + "\n";
        markdown += "```\n\n";
        if ((output as { explanation?: string }).explanation)
          markdown += `> ${(output as { explanation?: string }).explanation}\n\n`;
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 h-[calc(100vh-200px)]">
        ...
        <div className="md:col-span-3 flex flex-col h-full space-y-4">
          ...
          >
            ...
            >
              <div ref={setCanvasAreaRef} className="min-h-full space-y-4">
