...
type AdminPastDueRow = AdminMetrics["pastDue"][number];
...
async function generateAndCacheMetrics(): Promise<AdminMetrics> {
  ...
  const fetchPromise = (async () => {
    try {
      ...
      const mrr = (activeSubsByPlan as SubscriptionPlanAggregate[]).reduce(
        (total: number, entry) => {
      ...
      const newPaidMRR = (
        createdPaidInPeriod as SubscriptionPlanAggregate[]
      ).reduce((total: number, entry) => {
        return total + safeNumber(PLAN_PRICE_LOOKUP[entry._id]) * entry.count;
      }, 0);

      const cancelledPaidMRR = (
        cancelledPaidInPeriod as SubscriptionPlanAggregate[]
      ).reduce((total: number, entry) => {
        return total + safeNumber(PLAN_PRICE_LOOKUP[entry._id]) * entry.count;
      }, 0);
      ...
      const planBreakdown = (
        activeSubsByPlan as SubscriptionPlanAggregate[]
      ).map((entry) => ({
        planId: entry._id ?? "unknown",
        count: entry.count,
      }));

      const pastDue = (pastDueList as PastDueSubscription[])
        .map((item) => {
        ...
        .filter((item): item is AdminPastDueRow => item !== null);
...
export async function getAdminUsers(options: {
  ...
  const nextCursor = users.at(-1)?._id?.toString() || null;
  ...
  const usageByUser = new Map(
    (usage as UsageRecord[]).map((entry) => [entry.userId?.toString(), entry]),
  );
